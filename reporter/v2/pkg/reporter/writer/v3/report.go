...
func (r *ReportWriter) WriteReport(
	...
	metadata := schemav3alpha1.SourceMetadata{
		AccountID:     r.MktConfig.Spec.RhmAccountID,
		ClusterID:     r.MktConfig.Spec.ClusterUUID,
		Environment:   env,
		Version:       version.Version,
		ReportVersion: schemav3alpha1.Version,
	}
	...
	for idxRange := range gopart.Partition(len(metricsArr), partitionSize) {
		metricReport := &schemav3alpha1.MarketplaceReportSlice{}
		...
		for _, builder := range metricsArr[idxRange.Low:idxRange.High] {
			...
			if err != nil {
				...
				metricReport.Metrics = append(metricReport.Metrics, metric.(*schemav3alpha1.MarketplaceReportData))
			}
	...
	manifest := &schemav3alpha1.Manifest{
		Type:    schemav3alpha1.SWCAccountMetrics,
		Version: "1",
	}
