...
var _ = Describe("MeterDefinition", func() {
	...
	It("should convert to/from v1beta1", func() {
		...
		resourceFilter := MatchAllFields(Fields{
			...
			"WorkloadType": Equal(common.WorkloadTypePod),
		})
	...
	It("should convert the workload types back and forth", func() {
		Expect(ConvertWorkloadTypeAlpha(common.WorkloadTypePod)).To(Equal(WorkloadTypePod))
		Expect(ConvertWorkloadTypeAlpha(common.WorkloadTypeService)).To(Equal(WorkloadTypeService))
		Expect(ConvertWorkloadTypeAlpha(common.WorkloadTypePVC)).To(Equal(WorkloadTypePVC))
		Expect(ConvertWorkloadTypeBeta(WorkloadTypePod)).To(Equal(common.WorkloadTypePod))
		Expect(ConvertWorkloadTypeBeta(WorkloadTypeService)).To(Equal(common.WorkloadTypeService))
		Expect(ConvertWorkloadTypeBeta(WorkloadTypeServiceMonitor)).To(Equal(common.WorkloadTypeService))
		Expect(ConvertWorkloadTypeBeta(WorkloadTypePVC)).To(Equal(common.WorkloadTypePVC))
	})
