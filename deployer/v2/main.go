...
func main() {
	...
	*/
	if err = (&controllers.RazeeRRS3DeploymentReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("RazeeRRS3Deployment"),
		Scheme: mgr.GetScheme(),
	}).Inject(injector).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "RazeeRRS3Deployment")
		os.Exit(1)
	}
