			...
			It("should serve extra endpoints", func() {
				opts.Metrics.ExtraHandlers = map[string]http.Handler{
					"/debug": http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
						// x-remote headers should be passed on to this handler
						Expect(req.Header.Get("x-remote-user")).ShouldNot(BeEmpty())
						Expect(req.Header.Get("x-remote-groups")).ShouldNot(BeEmpty())
